...
void Tx_Char(unsigned char data)
{
	...
	for(i=0;i<8;i++)
	{
		SPI_CLK_LOW;			//first, set clock low
		delay_us();
		if((data<<i)&BIT7){
			SPI_SIMO_HIGH;		//send data 
		}
		else{
			SPI_SIMO_LOW;		//send data
		}
		delay_us();				//give some delay
		SPI_CLK_HIGH;			//set clock high, the data will be latched at rising edge
		delay_us();
	}
}
/******************************************************************************************************
